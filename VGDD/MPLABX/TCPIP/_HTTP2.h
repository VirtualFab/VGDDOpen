...
typedef enum {
    ...
    HTTP_FAT_UPLOAD_FORM, // Show the FAT Upload form
    HTTP_FAT_UPLOAD_UP, // A FAT Upload is being processed
    HTTP_FAT_UPLOAD_OK, // A FAT Upload was successful
    HTTP_FAT_UPLOAD_ERROR, // An error occured during FAT Upload
#endif
...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	typedef enum
	{
		...
		HTTP_AVI,			// File is AVI (extension .avi)
...
typedef struct {
    ...
    char Boundary[64];      // For multipart/form-data (HTPP 1.1 uploads) stores the unique boundary delimiter
    BYTE BoundaryLen;       // length of the boundary string found in POST Content-Type header
#endif
#if defined(STACK_USE_MDD) || defined(STACK_USE_FATFS)
    FILE_HANDLE pFhStoredFile;
    WORD nameHash ;
    FILE_HANDLE DynVarRcrdFilePtr;
    BOOL CurWorkDirChangedToMddRootPath;
    BYTE * directoryPtr;
    BYTE sendDataBuffer[64];
    SMSTATES smHTTPSendFile;
    BYTE lock;
    BYTE nameHashMatched;
    DWORD numBytes, dynVarCntr,dynVarRcrdOffset, dynVarCallBackID, bytesReadCount;
#endif
} HTTP_CONN;
