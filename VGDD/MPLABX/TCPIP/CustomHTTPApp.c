 ...
 * VirtualFab           5/19/2013  Modified for VGDD Demo + added HTTPPostUpload (HTTP 1.1 file upload support)
 ********************************************************************/
...
#include "vgdd_main.h"		// Needed for SaveAppConfig() prototype
...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	#endif
    #if defined(STACK_USE_MDD) && defined(STACK_USE_HTTP_UPLOADS)
        HTTP_IO_RESULT HTTPPostUpload(void);
    #endif
#endif
...
BYTE HTTPNeedsAuth(BYTE* cFile)
{
	...
	if(memcmppgm2ram(cFile, (ROM void*)"mpfsupload", 10) == 0)
		return 0x00;
        #elif defined(HTTP_FAT_UPLOAD_REQUIRES_AUTH)
	if(memcmppgm2ram(cFile, (ROM void*)HTTP_FAT_UPLOAD, sizeof (HTTP_FAT_UPLOAD)) == 0)
		return 0x00;
	#endif
...
BYTE HTTPCheckAuth(BYTE* cUser, BYTE* cPass)
{
	if(strcmppgm2ram((char *)cUser,(ROM char *) AppConfig.AdminUser) == 0
		&& strcmppgm2ram((char *)cPass, (ROM char *) AppConfig.AdminPassword) == 0)
...
HTTP_IO_RESULT HTTPExecuteGet(void) {
    ...
    #elif defined STACK_USE_MDD || defined STACK_USE_FATFS
    BYTE i, cntr1 = 0, cntr2 = 0, filext = 0;
    char *pFileName;
        #if defined STACK_USE_FATFS
    pFileName = (char *) FileGetNameFromHandle(curHTTP.file);
        #elif defined STACK_USE_MDD
    pFileName = curHTTP.file->name;
        #endif

    do {
        if (pFileName[cntr2] != 0x20) {
            filename[cntr1] = pFileName[cntr2];
            cntr1++;
        } else if (filext != TRUE) {
            filename[cntr1] = 0x2E;
            cntr1++;
    ...
    #else
        #error "No access method defined: either define STACK_USE_MPFS2 or STACK_USE_MDD or STACK_USE_FATFS"
    #endif
    ...
    if (!memcmppgm2ram(filename, "forms.htm", 9)) {
        ...
        if (*ptr >= '0' && *ptr <= '4') {
            LedState[*ptr - '0'] ^= 1;
            SwitchLedFromWeb(*ptr - '0');
        }
    } else if (!memcmppgm2ram(filename, "temp.cgi", 8)) { // If it's the temperature up/down updater file
        // Determine which button has been pushed
        ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *) "op");

        // Update temperature
        switch (*ptr) {
            case 'D': // Down
                if (intTemperature) intTemperature--;
                break;
            case 'U': // Up
                if (intTemperature < 30) intTemperature++;
                break;
        }
        TemperatureChanged();
    }
...
/*****************************************************************************
  Function:
	static HTTP_IO_RESULT HTTPPostTFT(void)

  Summary:
	Processes the message to display on TFT forms.htm

  Description:
	Locates the 'tft' parameter and uses it to update the text displayed
	on the board's TFT display.
	...
	is expected, it reads the associated value and writes it to the TFT.
...
static HTTP_IO_RESULT HTTPPostTFT(void)
{
	...
	#define SM_POST_TFT_READ_NAME		(0u)
	#define SM_POST_TFT_READ_VALUE		(1u)

	switch(curHTTP.smPost)
	{
		// Find the name
		case SM_POST_TFT_READ_NAME:

			// Read a name
			if(HTTPReadPostName(curHTTP.data, HTTP_MAX_DATA_LEN) == HTTP_READ_INCOMPLETE)
				return HTTP_IO_NEED_DATA;

			curHTTP.smPost = SM_POST_TFT_READ_VALUE;
			// No break...continue reading value

		// Found the value, so store the TFT and return
		case SM_POST_TFT_READ_VALUE:

			// If value is expected, read it to data buffer,
			// otherwise ignore it (by reading to NULL)
			if(!strcmppgm2ram((char*)curHTTP.data, (ROM char*)"tft"))
				cDest = curHTTP.data;
			else
				cDest = NULL;
			...
			if(!cDest)
			{
				curHTTP.smPost = SM_POST_TFT_READ_NAME;
				break;
			}

			// Copy up to 32 characters to the TFT
                        WriteMessageFromWeb(cDest);
...
HTTP_IO_RESULT HTTPExecutePost(void)
{
	...
	// Load the file name
	// Make sure BYTE filename[] above is large enough for your longest name
#if defined STACK_USE_MDD || defined STACK_USE_FATFS
	BYTE i,cntr1=0,cntr2=0,filext=0;
	BYTE cwdPath[20];
	...
	if(!memcmppgm2ram(filename, "upload.htm", 10)) {
#if defined(STACK_USE_MDD) && defined(STACK_USE_HTTP_UPLOADS)
		return HTTPPostUpload(); // Use the Upload Updates web page
#else              
            // Upload feature is not enabled, so redirect to a warning page
            strcpypgm2ram((char*)curHTTP.data, "/noupload.htm");
            curHTTP.httpStatus = HTTP_REDIRECT;
            return HTTP_IO_DONE;
#endif
        }
...
static HTTP_IO_RESULT HTTPPostConfig(void) {
	...
	// Use current config in non-volatile memory as defaults
        ...
        DataEEReadArray((unsigned char *) &newAppConfig, ADDRESS_TCPIP_APP_VERSION+4, sizeof (newAppConfig));
	...
	while(curHTTP.byteCount)
	{
        ...
        else if(!strcmppgm2ram((char*)curHTTP.data, (ROM char*)"secu"))
		{// Read new Security Mode
			newAppConfig.SecurityMode= curHTTP.data[6]-'0';
		}
		else if(!strcmppgm2ram((char*)curHTTP.data, (ROM char*)"ssid"))
		{// Read new SSID
			memcpy((void*)newAppConfig.MySSID, (void*)curHTTP.data+6, 16);
		}
#endif
	}
...
void HttpConnResetSm(void){
#ifdef STACK_USE_MDD
    curHTTP.nameHash = 0;
    curHTTP.DynVarRcrdFilePtr=INVALID_FILE_HANDLE;
    curHTTP.CurWorkDirChangedToMddRootPath=FALSE;
    curHTTP.directoryPtr=NULL;
    curHTTP.smHTTPSendFile=SM_IDLE;
    curHTTP.lock=0;
    curHTTP.nameHashMatched=0;
    curHTTP.numBytes=0;
    curHTTP.dynVarCntr=0;
    curHTTP.dynVarRcrdOffset=0;
    curHTTP.dynVarCallBackID=0;
    curHTTP.bytesReadCount=0;
#endif
}

/*****************************************************************************
  Function:
    static HTTP_IO_RESULT HTTPPostUpload(void)

  Summary:
    Processes the file upload form on upload.htm

  Description:
    This function demonstrates the processing of file uploads:
    the file is read 64 bytes at a time and stored on SD card

    It handles HTTP 1.1 multipart/form-data uploads in which POSTed data is separated
    by boundary tags. Boundaries are detected and file data is parsed and stored
    on configured media via FileXXXX functions (see FileSystem.c)

  Precondition:
    In the POSTed data the following fields are expected:
    upddir - The upload folder where the file will be stored. It can be OUT
             of the MDD_ROOT_DIR_PATH (WebPages) path. You can upload even in the root
             folder of the mounted media.
  ...
  Return Values:
    ...
    HTTP_IO_NEED_DATA - data needed by this function has not yet arrived
 ***************************************************************************/
#if defined(STACK_USE_MDD) && (defined(STACK_USE_HTTP_UPLOADS) || defined(HTTP_FAT_UPLOAD))

HTTP_IO_RESULT HTTPPostUpload(void) {
    UINT16 lenA, lenB, lenC;
    static char strFieldName[32];
    static char strUploadedFileName[32];
    static char strUploadFolder[32];
    static char strUploadPathName[64];
    char strBigBuffer[256];

    #define SM_UPD_PARSE_FIELDNAME	(0u)
    #define SM_UPD_PARSE_FIELDVALUE	(1u)
    #define SM_UPD_OPEN_FILE		(2u)
    #define SM_UPD_READ_DATA		(3u)

    while(curHTTP.byteCount>0) {
        // Read all browser POST data
        switch (curHTTP.smPost) {
            case SM_UPD_PARSE_FIELDNAME:
                curHTTP.pFhStoredFile=NULL;
                // See if a CRLF is in the buffer
                lenA = TCPFindROMArray(sktHTTP, "\r\n", 2, 0, FALSE);
                if (lenA == 0xffff) { //if not, ask for more data
                    return HTTP_IO_NEED_DATA;
                }
                // See if a field name is in the buffer
                lenB = TCPFindROMArray(sktHTTP, " name=", 2, 0, FALSE);
                if (lenB == 0xffff) { //if not, discard all data up to and including
                    lenA = TCPGetArray(sktHTTP, NULL, lenA + 4);
                    curHTTP.byteCount -= lenA;
                    break;
                }
                // Found name tag, find closing quote
                lenC = TCPFindROMArray(sktHTTP, "\"", 1, lenB+1, FALSE);
                if (lenC == 0xffff) { //if not, error
                    while(1);
                }
                lenC-=lenB; // Found it, compute field name length
                lenA = TCPGetArray(sktHTTP, NULL, lenB + 7); //  discard up to field name+starting quote
                curHTTP.byteCount -= lenA;

                // Get field name
                lenA = TCPGetArray(sktHTTP, strFieldName, lenC);
                strFieldName[lenC]=0; // NULL-terminate string
                curHTTP.byteCount -= lenA;

                // Locate end of field data (double CRLF, it's a multipart/form-data)
                lenA = TCPFindROMArray(sktHTTP, "\r\n\r\n", 4, 0, FALSE);
                // See if a filename is in the buffer, on current row
                lenB = TCPFindROMArray(sktHTTP, "filename=", 9, 0, FALSE);
                if ((lenB != 0xffff) && (lenB<lenA)) { // Found it, so discard data up to field name
                    lenB = TCPGetArray(sktHTTP, NULL, lenB + 10);
                    curHTTP.byteCount -= lenB;
                    lenA-=lenB;
                    lenB = TCPFindROMArray(sktHTTP, "\"", 1, 0, FALSE); // Find closing quote
                    TCPGetArray(sktHTTP, strUploadedFileName, lenB);
                    curHTTP.byteCount -= lenB;
                    strUploadedFileName[lenB]=0; // replace closing quote with NULL to terminate string
                    lenA-=lenB;
                    lenA = TCPGetArray(sktHTTP, NULL, lenA + 4); //  discard rest of field headers up to its value
                    curHTTP.byteCount -= lenA;
                    curHTTP.smPost = SM_UPD_OPEN_FILE; // Move to next state
                    break;
                }
                lenA = TCPGetArray(sktHTTP, NULL, lenA + 4); //  discard rest of field headers up to its value
                curHTTP.byteCount -= lenA;

                curHTTP.smPost=SM_UPD_PARSE_FIELDVALUE;
                // No break if we found the field name

            case SM_UPD_PARSE_FIELDVALUE:
                // See if a CRLF is in the buffer
                lenA = TCPFindROMArray(sktHTTP, "\r\n", 2, 0, FALSE);
                if (lenA == 0xffff) { //if not, ask for more data
                    return HTTP_IO_NEED_DATA;
                ...
                // Get field value
                if (!strcmppgm2ram(strFieldName, (ROM char*) "upddir")) { // Read destination folder name
                    lenA = TCPGetArray(sktHTTP, strUploadFolder, lenA );
                    strUploadFolder[lenA]=0;
                    TCPGetArray(sktHTTP, NULL, 2); //  discard final CRLF
                    curHTTP.byteCount -= lenA+2;
                }
                curHTTP.smPost=SM_UPD_PARSE_FIELDNAME; // Parse next field
                break;
                
            case SM_UPD_OPEN_FILE: // Try to open file for storage
                strUploadPathName[0] = 0;
#if defined(STACK_USE_MDD) && !defined(STACK_USE_FATFS)
                // For MDD with FSIO we need to change directory
                // and thus fix folder name replacing slashes with double backslashes (such a pain!)
                BYTE i=0;
                char c;
                char *pUploadFolder;
                curHTTP.CurWorkDirChangedToMddRootPath=FALSE; // Notify HTTP2_MDD that we changed current directory
                pUploadFolder=&strUploadPathName[0];
                while(c=strUploadFolder[i++]){
                    if(c=='/') {
                        *pUploadFolder++='\\';
                    } else {
                        *pUploadFolder++=c;
                ...
                if(FileChDir(strUploadPathName)!=0) {
                    if(FileMkDir(strUploadPathName)!=0) {
                        goto UploadFailure;
                    }
                    if(FileChDir(strUploadPathName)!=0) {
                        goto UploadFailure;
                ...
                curHTTP.pFhStoredFile = FileOpen(strUploadedFileName, "w");
#else
                // With FatFs we simply open the pathname (much better)
                // and slashes are correctly hadled too so simly concatenate folder and filename
                strcat(strUploadPathName, strUploadFolder);
                if(strUploadFolder[0]!='/' || strUploadFolder[1]!=0) // Do not append path separator if upload folder is root (/)
                    strcat(strUploadPathName, "/");
                strcat(strUploadPathName, strUploadedFileName);
                if(FileDirExists(strUploadFolder)!=0) {
                    if(FileMkDir(strUploadFolder)!=0) {
                        goto UploadFailure;
                    }
                }
                curHTTP.pFhStoredFile = FileOpen(strUploadPathName, "w");
#endif
                if (curHTTP.pFhStoredFile == NULL) {
                    goto UploadFailure;
                }
                curHTTP.smPost = SM_UPD_READ_DATA;
                // No break if we successfully opened file for writing

                // Read and store file data
            case SM_UPD_READ_DATA:
                // Find out how many bytes are available to be read
                // Locate end of file data (boundary tag, it's a multipart/form-data)
                lenC=0xffff;
                lenA=0;
                // Check only if the buffer has enough data and there is a CRLF in the buffer
                if(TCPIsGetReady(sktHTTP)>=curHTTP.BoundaryLen+2 && TCPFindROMArray(sktHTTP, (ROM BYTE*) "\r\n", 2, 0, FALSE)!=0xffff) {
                    lenC = TCPFindROMArray(sktHTTP, (ROM BYTE*) curHTTP.Boundary, curHTTP.BoundaryLen, 0, FALSE);
                    if(lenC!=0xffff) { // If found, Try to find start of boundary tag CRLF--------
                        lenB=TCPFindROMArray(sktHTTP, (ROM BYTE*) "\r\n----------", 12, 0, FALSE);
                        if(lenB==0xffff){ // Not found?
                            // Workaround: TCPFindROMArray does give up looking for "\r\n----" if "\r\n\r\n-----" is present (Content-Type: text/html)
                            // so let's retry with CRLFCRLF...
                            lenB=TCPFindROMArray(sktHTTP, (ROM BYTE*) "\r\n\r\n----------", 14, 0, FALSE);
                        }
                        if(lenB!=0xffff){ // FOUND!
                            lenA=lenB; // Use data only up to the boundary
                            // And compute complete boundary len to discard
                            lenC=lenB+(lenC-lenB)+curHTTP.BoundaryLen+2+2; // ------...-----<boundary>--CRLF
                        } else { // No CRLF---- but boundary IS in the buffer????
                            while(1);
                        }
                    }
                }
                if(lenA==0) { // If no boundary found
                    // Try to find a CR at end of buffer
                    lenA=TCPFindROMArray(sktHTTP, (ROM BYTE*) "\r", 1, TCPIsGetReady(sktHTTP)- 2, FALSE);
                    if(lenA==0xffff) { // If no CR at the end of the buffer
                        lenA=TCPIsGetReady(sktHTTP); // Then use all available bytes
                    }
                }
                ...
                while (lenA > 0u) {
                    //lenB = TCPGetArray(sktHTTP, strBigBuffer, (lenA < HTTP_MAX_DATA_LEN) ? lenA : HTTP_MAX_DATA_LEN);
                    lenB = TCPGetArray(sktHTTP, strBigBuffer, (lenA < sizeof(strBigBuffer)) ? lenA : sizeof(strBigBuffer));
                    if(lenB==0)
                        //goto UploadFailure;
                        break;
                    curHTTP.byteCount -= lenB;
                    lenA -= lenB;
                    if (FileWrite(strBigBuffer, 1, lenB, curHTTP.pFhStoredFile) != lenB) {
                        goto UploadFailure; // File write error
                    }
                }

                if (lenC != 0xffff) {
                    lenC = TCPGetArray(sktHTTP, strBigBuffer, lenC); //  discard boundary tag up to final CRLF
                    curHTTP.byteCount -= lenC;
                }
                if (curHTTP.byteCount > 0u)
                    return HTTP_IO_NEED_DATA; // Ask for more data
        }
    }
    if(curHTTP.pFhStoredFile!=NULL) // Close file
        FileClose(curHTTP.pFhStoredFile);
    curHTTP.pFhStoredFile=NULL;
    curHTTP.smPost = SM_UPD_PARSE_FIELDNAME; // Reset state machine for further uploads
    httpStubs[curHTTPID].sm = SM_HTTP_SERVE_HEADERS;
    HttpConnResetSm();
    return HTTP_IO_DONE;
...
#endif // #if defined(STACK_USE_HTTP_UPLOADS)
...
#if defined(WF_CS_TRIS)
void HTTPPrint_config_ssid(void)
{
	TCPPutROMString(sktHTTP, (ROM void*)AppConfig.MySSID);
}

void HTTPPrint_config_psk(void)
{
    BYTE i;
    char digit[3];
    for(i=0;i<32;i++) {
        sprintf(digit,"%02x",AppConfig.SecurityKey[i]);
        TCPPutROMString(sktHTTP, (ROM void*)digit);
    }
}

void HTTPPrint_security_options(void) {
    TCPPutROMString(sktHTTP,(ROM BYTE*)"<option value=\"0\"");
    if(AppConfig.SecurityMode==WF_SECURITY_OPEN)
		TCPPutROMString(sktHTTP, (ROM BYTE*)" selected");
    TCPPutROMString(sktHTTP,(ROM BYTE*)">Open (no security)</option>");
    
    TCPPutROMString(sktHTTP,(ROM BYTE*)"<option value=\"5\"");
    if(AppConfig.SecurityMode==WF_SECURITY_WPA2_WITH_KEY)
		TCPPutROMString(sktHTTP, (ROM BYTE*)" selected");
    TCPPutROMString(sktHTTP,(ROM BYTE*)">WPA2-PSK Personal</option>");
}
#endif
...
void HTTPPrint_updresult(void) {
    switch(curHTTP.data[0]) {
        case 99:
        	TCPPutROMString(sktHTTP, (ROM BYTE*)"<b>OK, file uploaded and stored</b><br>");
            break;
        case 101: 
        	TCPPutROMString(sktHTTP, (ROM BYTE*)"<b>Error: 101</b><br>");
            break;
        case 102: 
        	TCPPutROMString(sktHTTP, (ROM BYTE*)"<b>Error: 102</b><br>");
            break;
        case 103: 
        	TCPPutROMString(sktHTTP, (ROM BYTE*)"<b>Error: 103</b><br>");
            break;
    }
}

void HTTPPrint_authconfig_user(void) {
	TCPPutROMString(sktHTTP, (ROM void*)AppConfig.AdminUser);
}

void HTTPPrint_led(WORD num) {
	// Print the output
	TCPPut(sktHTTP, (LedState[num]?'1':'0'));
	return;
}

void HTTPPrint_ledSelected(WORD num, WORD state) {
	// Print output if TRUE and ON or if FALSE and OFF
	if((state && LedState[num]) || (!state && !LedState[num]))
		TCPPutROMString(sktHTTP, (ROM BYTE*)"SELECTED");
	return;
}
...
void HTTPPrint_Temperature(void) {
    BYTE AN0String[8];
    uitoa(intTemperature, (BYTE*) AN0String);
...
void HTTPPrint_tftmessage(void) {
    ...
    if(pTxt != NULL) {
        TCPPutString(sktHTTP, pTxt->pText);
    }
#endif
}

void HTTPPrint_webpages(void) {
    TCPPutString(sktHTTP, MDD_ROOT_DIR_PATH);
}
