...
#define PMP_CONTROL	(PMP_ON | PMP_MUX_OFF | PMP_READ_WRITE_EN|\
             		 PMP_WRITE_POL_LO | PCLK_POLARITY)

#define PMP_MODE     (PMP_DATA_LENGTH | PMP_MODE_MASTER2 |\
                     PMP_WAIT_BEG_1 | PMP_WAIT_MID_3 | PMP_WAIT_END_1 )
...
void ResetDevice(void)
{
    ...
    #else
    BACKLIGHT =1;
    DmaChnSetTxfer(1, (void*)&PMDIN ,&GraphicsFrame[0] , 1, HBackPorch, 1);
    #endif
    #endif
   ...
   OpenTimer2(T2_ON | T2_SOURCE_INT | T2_PS_1_1, 1); //Start Timer
   #endif
...
WORD scroll,scrollLine,scrollPage =0;
  ...
  volatile BYTE GraphicsState = 1;

#ifdef LCC_EXTERNAL_MEMORY
// handler for the DMA channel 1 interrupt
void __attribute__((weak)) __ISR(_DMA1_VECTOR, ipl7SRS) DmaHandler1(void)
{
            ...
            if(line++ >= -DISP_VER_FRONT_PORCH)
            {
             ...
             else
             {
               ...
               if((line >= PipStartT)&&(line <= (PipStartT + PipVLength))&&(PipVLength != 0))
               {
                     ...
                     PMADDR =  (pixelAddress);
                     ADDR16 = pixelAddress>>16;          //count holds the additional address line count
              ...
              else if(scroll >0)
              {
                 switch(scroll)
                 {
                  ...
                  case 2:             //Down
                   ...
                   ADDR16 = overflowcount.Val;          //count holds the additional address line count
                   break;
               ...
               break;
                        ...
                        {
                            ADDR16 = ++overflowcount.Val;          //count holds the additional address line count
                            ADDR17 = overflowcount.bits.b1;
                            ADDR18 = overflowcount.bits.b2;
                        }
                    #endif
...
static DWORD prevaddr;
    ...
    IEC0bits.INT0IE =   0;   // clear the interrupt flag
    ...
    IEC0bits.INT0IE = 1;   // clear the interrupt flag
...
static BYTE addressbit2,addressbit3;
    ...
    PMCONCLR=0x8100;   //  PMCONbits.PTRDEN=0;
    PMMODESET = 0x400;// PMMODEbits.MODE16 = 1;
    PMCONSET=0x8000;
...
//Save previous address value
    ...
    PMCONSET = 0x100; //PMCONbits.PTRDEN=1;
    ...
    PMMODECLR = 0x400;// PMMODEbits.MODE16 = 0;
