...
//#define CS_SETOUT() TRISBbits.TRISB7 = 0
...
//#define SPICONbits SPI2ACONbits
...
BYTE wait_ready(void) {
    ...
    rcvr_spi();
    do
        res = rcvr_spi(); while ((res != 0xFF) && Timer2);
...
#elif defined(USE_USB_INTERFACE) // FatFs on USB
...
if (MDD_SectorWrite(sector, (BYTE *) buff, FALSE) == TRUE)
        return RES_OK;
    else
        return RES_ERROR;
}
...
DSTATUS disk_initialize(BYTE drv) { // Physical drive nmuber (0)
    ...
    mediaInformation=(MEDIA_INFORMATION *)MDD_MediaInitialize();
...
DRESULT disk_read(
    ...
    if (MDD_SectorRead(sector, buff) == TRUE)
        return 0;
    else
        return 1;
}
...
DRESULT disk_ioctl(
    ...
    switch (ctrl) {
        case CTRL_SYNC: /* Flush dirty buffer if present */
            return (RES_OK); // TODO: Do something with USB stack to ensure flushing
            break;

        case GET_SECTOR_COUNT: /* Get number of sectors on the disk (WORD) */
            *(DWORD*) buff = 0; // TODO: Implement for f_mkfs()
            return (RES_OK);
            break;
