 ...
 * (the (c)Company(c)) for its PIC32 Microcontroller is intended
 * and supplied to you, the Company(c)s customer, for use solely and
 ...
 * license.
 *
 * THIS SOFTWARE IS PROVIDED IN AN (c)AS IS(c) CONDITION. NO WARRANTIES,
...
const unsigned int eedata_addr[NUM_DATA_EE_PAGES][NUMBER_OF_INSTRUCTIONS_IN_PAGE] 
// If you want to preserve DEE Flash contents:
//    1) reserve Flash address range in your programmer/debugger options (i.e. RealICE)
//        for PIC32MX795F512L: 0x1d03d000:0x1d03dfff
//    2) Use the following attribute placing the pages at a fixed (protected) address:
    __attribute__ ((aligned(4096),address(0x9d03d000)))={0};

// Otherwise, standard attribute (DEE Flash contents will be wiped oput at each Flash programming cycle
