...
Public Class TabControlEx
    ...
    Private LastTabAdded As TabPage

    Public Overloads Sub OnPaint()
    ...
    Protected Overrides Sub OnSelectedIndexChanged(e As System.EventArgs)
        ...
        RePositionCloseButton(LastTabAdded)
    End Sub
    ...
    Public Sub RePositionCloseButtons()
        For Each item As KeyValuePair(Of Button, TabPage) In CloseButtonCollection
            ...
            RePositionCloseButton(item.Value)
        Next
    End Sub

    Public Sub RePositionCloseButton(ByVal tp As TabPage)
        ...
        If btn IsNot Nothing Then
            ...
            If tpIndex >= 0 Then
                ...
                If Me.SelectedTab Is tp Then
                    ...
                    btn.Location = New Point(rect.X + rect.Width - btn.Width, rect.Y + 1)
                Else
                    btn.BackColor = Color.FromKnownColor(KnownColor.ButtonFace)
                    btn.Size = New Size(rect.Height - 3, rect.Height - 3)
                    'btn.Location = New Point(rect.X + rect.Width - rect.Height - 1, rect.Y + 1)
                    btn.Location = New Point(rect.X + rect.Width - btn.Width, rect.Y + 3)
                End If
    ...
    Protected Function CloseButtonOfTabPage(ByVal tp As TabPage) As Button
        For Each oItem As KeyValuePair(Of Button, TabPage) In CloseButtonCollection
            If oItem.Value Is tp Then
                Return oItem.Key
            End If
        Next
        Return Nothing
