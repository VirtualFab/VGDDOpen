    ...
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            private struct ContentStruct
            {
                ...
                private TabGradient m_ActiveGradient;
                public TabGradient ActiveGradient
                {
                    get { return m_ActiveGradient; }
                    set { m_ActiveGradient = value; }
                }

                private TabGradient m_InactiveGradient;
                public TabGradient InactiveGradient
                {
                    get { return m_InactiveGradient; }
                    set { m_InactiveGradient = value; }
                }
            }
            ...
            public static void GradientToXml(String GradientName, TabGradient Gradient, XmlTextWriter xmlOut)
            {
                if (Gradient != null)
                {
                    xmlOut.WriteAttributeString(GradientName + "_StartColor", ColorTranslator.ToHtml(Gradient.StartColor));
                    xmlOut.WriteAttributeString(GradientName + "_EndColor", ColorTranslator.ToHtml(Gradient.EndColor));
                    xmlOut.WriteAttributeString(GradientName + "_TextColor", ColorTranslator.ToHtml(Gradient.TextColor));
                }
            }
            ...
            public static void SaveAsXml(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                foreach (IDockContent content in dockPanel.Contents)
                {
                    ...
                    GradientToXml("ActiveGradient", content.DockHandler.ActiveGradient, xmlOut);
                    GradientToXml("InactiveGradient", content.DockHandler.InactiveGradient, xmlOut);
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn)
            {
                ...
                for (int i = 0; i < countOfContents; i++)
                {
                    ...
                    if (xmlIn.MoveToAttribute("ActiveGradient_StartColor"))
                    {
                        contents[i].ActiveGradient = new TabGradient();
                        contents[i].ActiveGradient.StartColor = ColorTranslator.FromHtml(xmlIn.GetAttribute("ActiveGradient_StartColor"));
                        contents[i].ActiveGradient.EndColor = ColorTranslator.FromHtml(xmlIn.GetAttribute("ActiveGradient_EndColor"));
                        contents[i].ActiveGradient.TextColor = ColorTranslator.FromHtml(xmlIn.GetAttribute("ActiveGradient_TextColor"));
                    }
                    if (xmlIn.MoveToAttribute("InactiveGradient_StartColor"))
                    {
                        contents[i].InactiveGradient = new TabGradient();
                        contents[i].InactiveGradient.StartColor = ColorTranslator.FromHtml(xmlIn.GetAttribute("InactiveGradient_StartColor"));
                        contents[i].InactiveGradient.EndColor = ColorTranslator.FromHtml(xmlIn.GetAttribute("InactiveGradient_EndColor"));
                        contents[i].InactiveGradient.TextColor = ColorTranslator.FromHtml(xmlIn.GetAttribute("InactiveGradient_TextColor"));
                    }
                    MoveToNextElement(xmlIn);
                }
                return contents;
            ...
            public static void LoadFromXml(DockPanel dockPanel, Stream stream, DeserializeDockContent deserializeContent, bool closeStream)
            {
                ...
                for (int i = 0; i < contents.Length; i++)
                {
                    ...
                    content.DockHandler.ActiveGradient=contents[i].ActiveGradient;
                    content.DockHandler.InactiveGradient = contents[i].InactiveGradient;
                }
